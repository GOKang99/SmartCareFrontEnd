...
const CistAdminPage = () => {
  ...
  const fetchCists = useCallback(async () => {
    try {
      ...
      let sortedCists = (data || []).sort(
        (a, b) => new Date(b.cisDt) - new Date(a.cisDt)
      );
      if (residentId !== "0") {
        sortedCists = sortedCists.filter(
          (cist) => cist.residentId === Number(residentId)
        );
      }
  ...
  const handleAddCist = async (newCist) => {
    try {
      const isDuplicate = cists.some(
        (cist) =>
          cist.cisDt === newCist.cisDt &&
          cist.residentId === Number(newCist.residentId)
      );
  ...
  const handleSelectResident = async (resId) => {
    try {
      setIsLoading(true);
      setResidentId(resId);
      const { data } = await api.get("/cist/admin/select", {
        params: { residentId: resId },
      });
      const sortedCists = (data || []).sort(
        (a, b) => new Date(b.cisDt) - new Date(a.cisDt)
      );
      setCists(sortedCists);
    } catch (error) {
